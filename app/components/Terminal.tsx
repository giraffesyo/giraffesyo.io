...
export default function Terminal() {
  ...
  const terminalWindow = (
    <div
      ref={windowRef}
      className={`rounded-xl overflow-hidden relative transition-all duration-300 w-full ${
        isFullscreen ? 'flex flex-col !rounded-none !h-screen !w-screen' : 'border border-stone-200 dark:border-zinc-800 shadow-2xl shadow-stone-200/50 dark:shadow-black/30'
      }`}
      style={
        isFullscreen
          ? { maxHeight: 'none' }
          : {
              maxHeight: windowState === 'closed' ? '0px' : '500px',
              opacity: windowState === 'closed' ? 0 : 1,
              borderWidth: windowState === 'closed' ? 0 : undefined,
            }
      }
    >
      {/* Title bar */}
      ...
      >
        ...
        <span className='text-[11px] font-mono text-stone-400 dark:text-zinc-600 ml-2'>
          ~
        </span>
      </div>
      ...
      <div
        ref={containerRef}
        className={`transition-all duration-300 bg-white dark:bg-zinc-950 ${isFullscreen ? 'flex-1 px-2 pt-1 [&_.xterm]:h-full [&_.xterm-screen]:h-full' : 'px-2 pt-1'}`}
        style={
          isFullscreen
            ? { overflow: 'hidden' }
            : {
                maxHeight: windowState === 'minimized' ? '0px' : '500px',
                opacity: windowState === 'minimized' ? 0 : 1,
                overflow: 'hidden',
                padding: windowState === 'minimized' ? '0 12px' : undefined,
              }
        }
      />
        ...
        >
          <div className='flex items-center gap-5 max-w-full'>
            ...
            <div className='font-mono text-xs leading-relaxed min-w-0'>
              ...
              <p className='text-stone-300 dark:text-zinc-700'>
                ──────────────────────
              </p>
              <p>
